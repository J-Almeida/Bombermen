...
public class Tests {
	...
	public void testEntityBuilder_Bomb() {
		...
		Bomb testBomb = new Bomb( EntityBuilder.CreateBomb(bombGuid, bombCreatorGuid, bombX, bombY) );
	...
	public void testEntityBuilder_Player() {
		...
		Player testPlayer = new Player( EntityBuilder.CreatePlayer(playerGuid, playerName, playerScore, playerX, playerY) );
	...
	public void testEntityBuilder_Wall() {
		...
		Wall testWall = new Wall( EntityBuilder.CreateWall(wallGuid, wallHitPoints, wallX, wallY) );
	...
	public void testEntityBuilder_PowerUp() {
		final int powerUpGuid = 40, powerUpType = 41;
		final float powerUpX = 42.0f, powerUpY = 43.0f;
		PowerUp testPowerUp = new PowerUp( EntityBuilder.CreatePowerUp(powerUpGuid, powerUpX, powerUpY, powerUpType) );
		
		assertEquals(powerUpGuid, testPowerUp.GetGuid());
		assertEquals(powerUpType, testPowerUp.GetType());
	...
	public void testEntityBuilder_Explosion() {
		...
		Explosion testExplosion = new Explosion( EntityBuilder.CreateExplosion(explosionGuid, explosionX, explosionY, explosionDir, explosionEnd) );
